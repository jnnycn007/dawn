...
matrix<float16_t, 3, 4> v_2(uint start_byte_offset) {
  ...
  return matrix<float16_t, 3, 4>(v_4, v_6, tint_bitcast_to_f16(select(((((16u + start_byte_offset) & 15u) >> 2u) == 2u), v_7.zw, v_7.xy)));
}
...
void f() {
  ...
  vector<float16_t, 4> l_a_i_a_i_m_i = tint_bitcast_to_f16(select((((((v_23 + v_24) + v_25) & 15u) >> 2u) == 2u), v_26.zw, v_26.xy));
