...
matrix<float16_t, 4, 4> v_3(uint start_byte_offset) {
  ...
  vector<float16_t, 4> v_7 = tint_bitcast_to_f16_1(select(((((8u + start_byte_offset) & 15u) >> 2u) == 2u), v_6.zw, v_6.xy));
  uint4 v_8 = u[((16u + start_byte_offset) / 16u)];
  vector<float16_t, 4> v_9 = tint_bitcast_to_f16_1(select(((((16u + start_byte_offset) & 15u) >> 2u) == 2u), v_8.zw, v_8.xy));
  uint4 v_10 = u[((24u + start_byte_offset) / 16u)];
  return matrix<float16_t, 4, 4>(v_5, v_7, v_9, tint_bitcast_to_f16_1(select(((((24u + start_byte_offset) & 15u) >> 2u) == 2u), v_10.zw, v_10.xy)));
}
