...
matrix<float16_t, 4, 3> v_2(uint start_byte_offset) {
  ...
  vector<float16_t, 3> v_8 = tint_bitcast_to_f16_1(select(((((16u + start_byte_offset) & 15u) >> 2u) == 2u), v_7.zw, v_7.xy)).xyz;
  uint4 v_9 = u[((24u + start_byte_offset) / 16u)];
  return matrix<float16_t, 4, 3>(v_4, v_6, v_8, tint_bitcast_to_f16_1(select(((((24u + start_byte_offset) & 15u) >> 2u) == 2u), v_9.zw, v_9.xy)).xyz);
}
...
void f_inner(uint tint_local_index) {
  ...
  matrix<float16_t, 4, 3> v_16[4] = v_10(0u);
  w = v_16;
  w[1u] = v_2(64u);
