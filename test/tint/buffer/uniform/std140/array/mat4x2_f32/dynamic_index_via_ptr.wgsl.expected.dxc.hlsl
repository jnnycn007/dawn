...
[numthreads(1, 1, 1)]
void f() {
  uint v_9 = (min(uint(i()), 3u) * 32u);
  uint v_10 = (min(uint(i()), 3u) * 8u);
  float4x2 l_a[4] = v_5(0u);
  float4x2 l_a_i = v(v_9);
  uint4 v_11 = a[((v_9 + v_10) / 16u)];
  float2 l_a_i_i = asfloat(select(((((v_9 + v_10) & 15u) >> 2u) == 2u), v_11.zw, v_11.xy));
  s.Store(0u, asuint((((asfloat(a[((v_9 + v_10) / 16u)][(((v_9 + v_10) & 15u) >> 2u)]) + l_a[0u][0u].x) + l_a_i[0u].x) + l_a_i_i.x)));
}
