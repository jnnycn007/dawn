...
matrix<float16_t, 3, 4> v_1(uint start_byte_offset) {
  ...
  return matrix<float16_t, 3, 4>(v_3, v_5, tint_bitcast_to_f16(((((((16u + start_byte_offset) & 15u) >> 2u) == 2u)) ? (v_6.zw) : (v_6.xy))));
}

[numthreads(1, 1, 1)]
void f() {
  uint v_7 = (min(uint(i()), 2u) * 8u);
  matrix<float16_t, 3, 4> l_m = v_1(0u);
  uint4 v_8 = m[(v_7 / 16u)];
  vector<float16_t, 4> l_m_i = tint_bitcast_to_f16((((((v_7 & 15u) >> 2u) == 2u)) ? (v_8.zw) : (v_8.xy)));
}
